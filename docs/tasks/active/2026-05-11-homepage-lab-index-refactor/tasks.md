...
Overall status: implementation complete; standalone `/projects` page removed by owner follow-up; T9 doctrine update remains gated on user acceptance
...
- evidence:
  ...
  - Browser 1440px checks: homepage nav inner x=40 width=1360, lab shell x=40 width=1360, footer inner x=40 width=1360; resume shows the same shell alignment; blog post shell x=40 width=1360 while prose remains width=820.
  - Browser 390px checks across `/`, `/resume`, and `/blog/on-how-to-write-software/`: document scrollWidth equals viewport width, no horizontal overflow.
- blockers: []
...
- evidence:
  ...
  - Follow-up projects polish: homepage artifact rows now top-align metadata, titles, descriptions, and tags; project list framing was reduced to only the row-to-row divider.
  - Browser 390px: hero copy, ledger, artifact row, notes, sidebox, and archive all stack at width=354 with scrollWidth=390.
  - Screenshot evidence: `evidence/homepage-desktop-lab-index.png`, `evidence/homepage-mobile-lab-index.png`, `evidence/homepage-projects-before-alignment.png`, `evidence/homepage-projects-after-desktop.png`, `evidence/homepage-projects-after-mobile.png`.
- blockers: []
- notes: Use the reference as visual/spec guidance, not as raw copy-paste if Astro data binding requires adaptation.

## T3 Wire homepage project artifacts and decide data ownership mechanically
- status: done
- deps: ["T2"]
- intent: Expose projects on the homepage using consistent factual content.
- target: `src/pages/index.astro`, `src/data/labArtifacts.ts`
- acceptance:
  ...
  - Project facts preserve the accepted homepage project rows unless the user explicitly changes copy.
  - Project facts live in a small shared data module to keep homepage rendering data-driven.
  - Project rows have hover/focus behavior consistent with the brutalist accent style.
- verification:
  - Manual source review of homepage project facts.
  - Browser click/focus check on project links.
  - `npm run build`
- evidence:
  - Extracted shared project facts to `src/data/labArtifacts.ts`; after the standalone `/projects` page was removed, this module was trimmed to the homepage fields.
  - Homepage renders MyDanceDNA and No Noise Letter with artifact id/status, title, description, stack tags, and external links.
  - Homepage project links resolve to `https://www.mydancedna.com/` and `https://nonoiseletter.com/`.
- blockers: []
...
- verification:
  ...
  - `npm run build`
- evidence:
  - Historical before supersession:
  ...
  - Owner follow-up superseded this task: standalone `/projects` route was removed because the homepage projects section is sufficient.
- blockers: []
...
- target: `/`, `/resume`, one `/blog/<slug>/`, removed `/projects` route, task evidence files
- acceptance:
  - Desktop homepage shows wider lab-index layout without crowding or agency-style oversized cards.
  - Desktop homepage/resume/post pages feel like the same family while preserving page-specific readability and function.
...
- evidence:
  ...
  - Before route removal, final post-cleanup iframe audit repeated those four routes at 1440x1000 and 390x844 against the current dev server; every route reported `overflow=false`, homepage latest count 5, homepage archive links 26, and blog post ToC links 53.
  - Follow-up homepage projects audit at 1440x1000: project section width=1360; artifact child tops match within each row; first row has no top/bottom frame divider; second row keeps one top divider. At 390x844: project section width=354, document scrollWidth=390, overflow=false.
  - Follow-up route removal: `/projects` now hits the repo 404 route, which redirects to `/`; `dist/projects/index.html` is absent; main nav no longer contains a Projects link.
  - Relevant boxes checked separately: nav content, lab shell, hero/header copy, remaining summary ledger/resume rail, artifacts, notes/archive, resume rail/content rows, blog post shell, ToC, article, prose, actions, footer.
